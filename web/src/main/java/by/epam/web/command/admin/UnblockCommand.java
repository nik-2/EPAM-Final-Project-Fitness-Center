...
/**
 * The type Unblock command.
 */
public class UnblockCommand implements ActionCommand {
    private static final Logger logger = LogManager.getLogger(UnblockCommand.class);

    private DataBaseUserDao dataBaseUserDao = DaoFactory.getInstance().getUserDao();

    @Override
    public Router execute(HttpServletRequest request) {
        logger.debug("Start unblock command");
        ...
        session.setAttribute(StringConst.PARAM_NAME_CHANGE, StringConst.PARAM_NAME_UNBLOCK);
        ...
        try {
            ...
            logger.error("Trouble with unblock command", e);
