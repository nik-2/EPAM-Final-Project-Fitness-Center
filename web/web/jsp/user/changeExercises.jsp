...
<head>
    ...
    <title><fmt:message key="label.user.changeExercise"/></title>
    ...
    <ul>
        <li>
            <form name="englishForm" method="GET" action="controller">
                ...
                <input type="hidden" name="page" value="/jsp/user/changeExercises.jsp"/>
...
<h2><fmt:message key="label.user.myExercises"/></h2>
<div class="errorMess">${param.errorMess}</div>
<div id="changeBtn">
    <button type="submit" onclick="changeExercise()" class="editData"><b>&#128393;</b><b class="edit"> <fmt:message key="label.user.CHANGEEXERCISE"/> </b></button>
</div>
<div class="dataContent">
    <div id="responseBtn">
        <form class="responseBtn" name="cancel" method="GET" action="controller">
            <button type="submit" class="cancelbtn" name="command" value="view-exercise"><fmt:message key="label.start.cancelBtn"/></button>
        </form>
        <form onsubmit="return checkValidation(document.getElementById('exercise'))" class="responseBtn" name="confirm" method="POST" action="controller">
            <input type="hidden" id="newExercise" name="exercise" value="${exercise}">
            <button type="submit" class="confirmbtn" name="command" value="change-user-exercise"><fmt:message key="label.start.confirm"/></button>
        </form>
    </div>

    <div class="inp">
        <textarea type="text" class="dataContentInput" onchange="setNewExercise()" onfocus="checkDietAndExer(this)" oninput="checkDietAndExer(this)"
                  id="exercise" cols="100" rows="6" maxlength="600" readonly>${exercise}</textarea>
    </div>
</div>
</body>
</html>